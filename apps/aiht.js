...
export class aiht extends plugin {
  ...
  async huatu2(e) {
    ...
    if (kg2 == 0 & e.isGroup | e.isMaster) {
      ...
      if (reg.test(data1)) {
        ...
        if (res4 == '') {
          let url3 =
            ' http://www.iinside.cn:7001/api_req?reqmode=nmt_mt5_jez&password=3652&text=' +
            gjc + '&order=zh2en'
      ...
      var str = "abc";
      var data = [
        res4, pc, 'None', 'None', 20, "Euler a", false, false, 1, 1, 7, -1,
        -1, 0, 0, 0, false, changdu, kuandu, false, false, 0.7, 'None',
        false, false, null, '', "Seed", '', "Steps", '', true, false, null,
        '', ''
      ]
      var data2 = [
        res4, pc, "None", "None", 28, "Euler a", false, false, 1, 1, 11, -1,
        -1, 0, 0, 0, false, 512, 512, false, false, 0.7, "None", false,
        false, null, "", "Seed", "", "Steps", "", true, false, null, '', ''
      ]
      //  let url = dz
      //    try {
      //      res = await fetch(url, {
  ...
  async huatu(e) {
    ...
    if (kg == 0 & e.isGroup) {
      ...
      if (res4 ==
        '{"error":"Too many requests currently processing, please try again later"}'
      ) {
        e.reply('对面服务器很忙，等等')
        kg = 0
        return
